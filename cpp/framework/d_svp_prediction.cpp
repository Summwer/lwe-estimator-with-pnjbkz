...
tuple<int,int,double,double> fixed_dsvp_predict(vector<double> l, double cum_pr, COST* cost, int cost_model){
    /*
    return dsvp, G, B. 
    */
    int d = l.size();
    ...
    for(int dsvp = 50; dsvp <= d; dsvp++ ){
        ...
        if(rp < 0.001){
            ...
            // if(not worst_case)
            return  make_tuple(dsvp_, dsvp, p_cost.first * ((1-cum_pr) * psvp + rp), p_cost.second  * ((1-cum_pr) * psvp + rp));  //Avoid too small of dsvp
            // else
...
// tuple<int,int,double,double> progressive_dsvp_predict(vector<double> l, double cum_pr, COST* cost, int cost_model, bool worst_case){
//     /*
//     return dsvp, G, B. 
//     */
//     int d = l.size();
