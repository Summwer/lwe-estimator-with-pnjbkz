...
def svp_estimate_attack(silent=False, method=1, worst_case = False, parallel_ = False, l = None, dvol = None, dim_ = None, J=1, gap=1, J_gap = 1, cost_model = 1, gen_GSA_gso = True,print_l = False, b = None, sigma = 0., ldc_param = "AGPS20", cal_ee = "chi", goal_min_cost = "gate_min", cumG = False):
    ...
    if(method==4):
        print(" Attack Estimation via default g6k mode  ", end="")
        if(cost_model == 1):
            print(", list_decoding_classical = ", ldc_param)
        print()
    ...
    if not silent:
        if cost_model == 1:
            ...
            time_unit = "G = %3.2f log2(sec) \t B = %3.2f log2(avg_GB)" %(G, (B-33))
        if method == 1:
...
def cvp_estimate_attack(silent=False, method=1, worst_case = False, parallel_ = False, l = None, dvol = None, dim_ = None, J=1, gap=1, J_gap = 1, cost_model = 1, gen_GSA_gso = True,print_l = False, b = None, sigma = 0., ldc_param = "AGPS20", cal_ee = "chi", goal_min_cost = "gate_min", cumG = False):
    """ Assesses the complexity of the lattice attack on the instance.

    method = 1: cvp estimate: colattice
             2: batch-cvp estimate
    ldc_param: classical list decoding complexity, choose  "AGPS20"(https://eprint.iacr.org/2019/1161.pdf) or "MATZOV22"(https://marketing.idquantique.com/acton/attachment/11868/f-0587a79f-5592-47fe-9bdf-a3f3e7f7d802/1/-/-/-/-/Report%20on%20the%20Security%20of%20LWE.pdf)
