...
def lwechal_est(method,  cost_model, worst_case,J=1, gap=1, J_gap=1, parallel_ = False,  print_l = False, lwechals = lwechals, default_g6k=False, ldc_param = "AGPS20"):
    for (n,alpha) in lwechals:
        (l,dim,dvol,b,sigma) = gen_lwechal_instance(n, alpha,default_g6k)
        estimate_attack( silent=False, method = method,  parallel_ = parallel_, l = l, dim_ = dim, dvol =dvol, J=J,gap =gap,J_gap = J_gap, cost_model=cost_model,gen_GSA_gso = False, print_l = print_l, worst_case = worst_case,b = b, sigma = sigma, ldc_param = ldc_param)
