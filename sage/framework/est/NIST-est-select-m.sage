...
def initialize_from_LWE_instance_in_two_step(n, q, m, D_e, D_s, verbosity = True,  J=1, gap=1, J_gap=1, l = None, gen_GSA_gso = True, parallel_ = False,  print_l = False, ldc_param = "AGPS20", cal_ee = "chi", worst_case = False, goal_min_cost = "gate_min", cumG = False):
    ...
    for test_m in range(m,0,-1):
        ...
        d = test_m+n+1
        #G = compute_beta(d, dvol)
        G, B = estimate_attack( silent=False, method = method, parallel_ = parallel_, l = l, dvol = dvol, dim_ = d, gen_GSA_gso=gen_GSA_gso,print_l = print_l, ldc_param = ldc_param, cal_ee = cal_ee, worst_case = worst_case, goal_min_cost = goal_min_cost, cumG = cumG)
        Gs.append(G)
        if(dvol <= 0 or G is None):
            break
        if(min_G == - 1 or min_G > G ):
            min_G = G
            min_B = B
            best_m = test_m
    ...
    dim = m + n + 1
    print("dim = %d, m = %d, dvol = %3.11f, minG = %3.4f" %(dim, m, dvol, min_G))
    print("All G:", Gs)
