...
def initialize_from_LWE_instance_in_two_step(n, q, m, D_e, D_s, verbosity = True,   l = None, gen_GSA_gso = True, parallel_ = False,  print_l = False, ldc_param = "AGPS20", cal_ee = "chi", worst_case = False, goal_min_cost = "gate_min", cumG = False):
    ...
    for test_m in range(m,0,-1):
        ...
        G, B = estimate_attack( silent=True, method = method, parallel_ = parallel_, l = l, dvol = dvol, dim_ = d, gen_GSA_gso=gen_GSA_gso,print_l = print_l, ldc_param = ldc_param, cal_ee = cal_ee, worst_case = worst_case, goal_min_cost = goal_min_cost, cumG = cumG)
        Gs.append(G)
